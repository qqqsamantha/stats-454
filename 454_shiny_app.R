...
ui<-fluidPage(
  ...
  sidebarLayout(position = "left",
                ...
                mainPanel("by Xinyi Wang and Yiyang Shi",
                          ...
                          strong("Bayesian Logistic Regression Model"),
                          ...
                          withMathJax(helpText(p("$$\\text{ ... }$$"))),
                          withMathJax(helpText(p("$$\\beta_{26}    \\sim N(m_{26}, s_{26}^2)$$"))),
                          strong("Note:"),
                          withMathJax(helpText(p("The priors on $$(\\beta_0, \\beta_1, ... \\beta_{26})$$ are independent of one another."))),
                          withMathJax(helpText(p("For interpretability, we state our prior understanding of the model intercept $$\\beta_0$$ through the **centered intercept** $$\\beta_{0c}$$."))),
                          withMathJax(helpText(p("When $$X = 0$$, $$\\beta_0$$ is the expected *logged* odds for the hotel booking is canceled and $$e^{\\beta_0}$$ is the expected *odds* for the hotel booking is canceled"))),
                          withMathJax(helpText(p("For each 1-unit increase in $$X_1$$, $$\\beta_1$$ is the expected change in the *logged odds of the hotel booking is canceled* and $$e^{\\beta_1}$$ is the expected *multiplicative* change in *odds*."))),
                          withMathJax(helpText(p("$$\\beta_1 = \\log(\\text{odds}_{x_1+1}) - \\log(\\text{odds}_{x_1}) \\;\\; \\text{ and } \\;\\; e^{\\beta_1} = \\frac{\\text{odds}_{x_1+1}}{\\text{odds}_{x_1}}$$"))),
                          withMathJax(helpText(p("Same formula will apply to the remaining predictors $$\\beta_2, ..., \\beta_{26}$$"))),
                          
                          strong("Bayesian Lasso Logistic Regression Model"),
                          withMathJax(helpText(p("$$Y_i | \\beta_0, \\beta_1,...,\\beta_{26}, \\sigma  \\stackrel{ind}{\\sim} \\text{Bern}(\\pi_i)$$"))),
                          withMathJax(helpText(p("$$\\beta_{k}  \\sim \\text{Laplace}(0, 1/\\lambda)$$"))),
                          strong("Note:"),
                          withMathJax(helpText(p("The priors on $$\\beta_k$$ are independent of one another. We set a laplace prior because the laplace distribution centered at 0, meaning if the coefficient shrink to 0, the corresponding variable will not be considered as one of the predictors in the model. "))),
                          withMathJax(helpText(p("$$\\beta_k$$ represent the coefficient of predictors $$X_{1},X_{2}...X_{26}$$ and we also assume that each of the variable follow a laplace distribution."))),
                          
                          p("Horseshoe Prior"),
                          withMathJax(helpText(p("$$Y_i | \\beta_0, \\beta_1,...,\\beta_{26}, \\sigma  \\stackrel{ind}{\\sim} \\text{Bern}(\\pi_i)$$"))),
                          withMathJax(helpText(p("$$\\beta_{k}|\\tau,\\lambda  \\sim \\text{N}(0, \\tau\\lambda_k)$$"))),
                          withMathJax(helpText(p("$$\\lambda_k  \\sim \\text{HalfCauchy}(0,1)$$"))),
                          withMathJax(helpText(p("$$\\tau  \\sim \\text{HalfCauchy}(0,\\tau_0)$$"))),
                          strong("Note:"),
                          withMathJax(helpText(p("The prior on $$\\beta_k$$ is a model which under the condition of $$\\lambda_k$$ and $$\\tau$$"))),
                          withMathJax(helpText(p("The heavy-tailed Cauchy prior on $$\\lambda_k$$ allows individual coefficients to offset and global shrinkage."))),
                          withMathJax(helpText(p("$$\\tau$$ shares the similar heavy-tailed cauchy prior with $$\\lambda_k$$, but we need to choose $$\\tau_0$$ to determinate how fast the model shrink to 0."))),
