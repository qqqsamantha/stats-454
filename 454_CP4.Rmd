...
> Since our final product would be a Shiny. So that we include some screenshots of our app in the later parts and those would serve as our data visualization.
...
> Horseshoe prior will shrink the weight of those unimportant variables to 0 so with the mcmc_interval above, we can eliminate the variables whose interval shrinks to zero, meaning they don't have plenty of contribution on predicting the hotel cancellation. We will eliminate the variables: lead_time, adults, babies, stays_in_weekend_nights, stays_in_week_nights, previous_bookings_not_canceled, booking_changes, adr, reserved_room_type, meal

```{r,eval=FALSE}
hotel_hs <- hotel %>% 
  select(-lead_time,-adults, -babies, -stays_in_weekend_nights,-stays_in_week_nights,-previous_bookings_not_canceled,-booking_changes,-adr,-reserved_room_type,-meal,-reservation_status, -reservation_status_date,-company, -arrival_date_day_of_month, -days_in_waiting_list, -agent) %>% 
  filter(country %in%
           c("AUT","BEL","BRA","CHE","CHN","CN","DEU","ESP","FRA","GBR","IRL",
             "ISR","ITA","NLD","NOR","POL","PRT","RUS","SWE","USA")) %>% 
  mutate(is_canceled = factor(is_canceled),
...
saveRDS(log_hs_model_updated,file=here("models","log_hs_model_updated.rds"))
```
...
> Then we perform a cross validation and confirms our log_hs_model_updated model is a good one.

## ShinyApp

> Here we only have a baseline ShinyApp and more graphs should be added.

#The link: https://qqqsamantha.shinyapps.io/454_project/

> Some screenshots are provided below:

![screenshot1](/Users/apple/Desktop/454_project/screenshot1.png)

![screenshot2](/Users/apple/Desktop/454_project/screenshot2.png)

## Contributions:
> Yiyang and Xinyi contribute equally on this checkpoint. Most of the work are done together. We decide which variables to delete together. Then Yiyang forcused on running the updated model and save as the RDS file. He also write out the notation for Batesian logistic model. However, we are both not sure how to write out the horseshoe and Laplace model notation so futher work is needed. Xinyi mainly forcused on using the model from RDS to make predictions and building the shinyApp. Finally, we come together and write out this checkpoint together and discuss about our next step.
